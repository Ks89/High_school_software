...
var
   ...
   name1,name2,pesca,ok,girare1,girare2:string;
...
repeat          {questo repeat è quello che fa terminare il gioco se ho fatto tot partite quante sono quelle scelte per il torneo}
   ...
   if bugx003=true then writeln(' A causa del bug x003 è necessario riavvire 7 e mezzo');
...
if pesca='s' then begin
 ...
 if (coperta1=false) then begin
  repeat
  write('Puoi pescare solo carte coperte [c]?: ');
  textcolor(white);
  readln(pesca);
  if pesca='S' then pesca:='s';
  ...
  until (pesca='c');
      ...
      repeat
      write('Puoi prendere solo carte scoperte  [s]?: ');
      textcolor(white);
      readln(pesca);
      if pesca='S' then pesca:='s';
      if pesca='C' then pesca:='c';
 ...
 if pesca ='c' then coperta1:=true;
  ...
  if (pesca='c') and (giro1=12) then begin
   ...
   writeln;
 if (p1+c1<7.5) or (pesca<>'n') then begin
   textcolor(lightmagenta);
   write(name1,' ');
   textcolor(yellow);
...
if pesca='s' then begin
 ...
 if (coperta2=false) then begin
  ...
  if (pesca='c') and (giro2=12) then begin
   ...
   writeln;
 if (p2+c2<7.5) or (pesca<>'n') then begin
    textcolor(lightgreen);
    write(name2,' ');
    textcolor(yellow);
    write('vuoi girare la carta coperta [s]?');
