...
//NOVITA':
//-Aggiunti md5 e sha su file irr e ks
//-correzione bug
//-Continui test su attivazione eccc... hanno dato risultati positivi
//-Migliorata la compatibilità con LCL beta 2.0.0 che è stato modificato per interfacciarsi correttamente con beta 17.2.0
...
void hash_rex_dll_funzione(char path_fun[],char nomepc_fun[],int codice_fun[],int controllo_fun,int controlloic_fun[])
{
   ...
   printf("\n %d   indice1 %c \n",ind,stringa_hash[ind+lungh_hash-1],stringa_hash[ind+lungh_hash],stringa_hash[ind+lungh_hash+1]);
   ...
   printf("\n %c %c %c \n",stringa_hash[ind+12-30+lungh_hash],stringa_hash[12-1+ind+lungh_hash],stringa_hash[12+ind+lungh_hash]);
   ...
   printf("\n %c \n",stringa_hash[ind+lungh_hash]);
   printf("\n %s \n",stringa_hash);
   printf("\n Controlloic: ");
   for(ind=0;ind<30;ind++) printf("%d ",controlloic_fun[ind]);
   printf("\n\n\n");
					...
					do{ //eseguo almeno una volta la generazione del codice, ma se alla fine mi creasse numeri non ammessi nella risposta rigenero il codice finchè le condizioni non mi vanno bene
						...
						if ((cfPtr=fopen("lic.dat","w"))==NULL) printf("Errore x009c\n");
					         ...
					         if ((cfPtr=fopen("lic.dat","rb+"))==NULL) printf("Errore x009d\n");
									...
									fseek(cfPtr,0,SEEK_SET);
									fwrite(&activate,sizeof(struct activateData),1,cfPtr); 
									fclose(cfPtr);  
                             } 
				        }	
...
printf("             *****************************************************\n");
printf("             *..::Benvenuto in LIGHTWALL BETA 17.2.0 by ks89!::..*\n");
printf("             *****************************************************\n\n");
