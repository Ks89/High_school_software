...
//VERSIONE INNOVATIVA, MA ANCORA NON COMPLETA. QUESTA E' PERFETTA PER CODICE DI RISPOSTA E L'HASH SU ALTRI 2 FILE SEMBRA FUNZIONARE, MA MOLTE COSE DEVONO ANCORA ESSERE IMPLEMENTATE.
...
//Richiede obbligatoriamente LCL beta 2.0.x e anche GENERAZIONE RISPOSTA BETA 5 (le versioni prima non funzionano)
...
//NOVITA':
//-nuova versione licenza, passaggio alla versione 2.0.1 in quanto ho iniziato ad aggiungere le parti per l'upgrade ec...
   //TUtte le prossime modifiche cambieranno la versione 2.0.x incrementando solo l'utlima cifra, perchè saranno modifiche che porteranno alla 2.1.0
//-implementato il sistema UPGRADE licenza, ancora rudimentale, ma davvero funzionante e testato abbastanza
...
void hash_license_funzione(int codice_fun[],int controllo_fun,char nome_fun[],char cogn_fun[],char soc_fun[],char mail_fun[],char reg_fun[],int num_domanda_fun,int domanda_fun[],char version_fun[])
{
   ...
   for(ind=0;ind<50;ind++) if(domanda_fun[ind]!=0) {
													stringa_hash[lun_part2+ind]=domanda_fun[ind];
													printf("%c ",domanda_fun[ind]);
													}
...
printf("             *****************************************************\n");
printf("             *..::Benvenuto in LIGHTWALL BETA 17.3.0 by ks89!::..*\n");
printf("             *****************************************************\n\n");
...
if (scelta==1){
              ...
              if ((cfPtr=fopen("license.lic","rb"))==NULL) printf("File di licenza non trovato. Errore x015\n");
				...
				if ((cfPtr=fopen("lic.dat","rb"))==NULL) printf("File lic non trovato. Errore x016\n");
					...
					printf("\nMD5: ");
					for (i=0;i<16;i++) printf ("%02x", md5_fun2[i]);
					printf("\nSHA-1: ");
					for (i=0;i<20;i++) printf ("%02x", sha1_fun2[i]); 
					printf("\ncontr hash_ %d %d\n",contr_sha_license,contr_md5_license);
...
if (scelta==4)
{
	...
	{//se è permesso fare l'upgrade entro qui, cioè solo se la licenza è home
		...
		{//se entro qui vuol dire che la licenza ha un codice corretto e posso stampare il codice seriale della licenza
			...
			int upgrade=0;
			printf("\nChe upgrade vuoi fare?\n1-Inserisci un numero da 1 a 3 per aumentare il numero di anni di validità\n2-Inserisci 0 per passare alla versione PRO\n\n");
			scanf("%d",&upgrade);
			...
			else
			{
				...
				{	//questo fa si che se voglio fare upgrade, la licenza scadrà l'anno in cui ho registrato la licenza la prima volta +1(2 o 3)
					...
					if(upgrade==1) {
									license.anno_s=license.anno_r+1;
									//license.tipolic=1;
								   }
					if(upgrade==2) {
									license.anno_s=license.anno_r+2;
									//license.tipolic=2;
								   }
					if(upgrade==3) {
									license.anno_s=license.anno_r+3;
									//license.tipolic=3;
								   }
