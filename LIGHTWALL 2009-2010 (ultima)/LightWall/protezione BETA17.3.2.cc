...
//NOVITA':
//-Possibile rinnovare la licenza anche dopo la scadenza con apposito codice
//-Miglioramento compatibilità con LM
//-Ordine codici errori (tutti in numeri x000 oppure 2 particolari irreversibili su generaz percorso e nome pc indicati con x00A e x00B
//-Pulizia codice sorgente da printf ecc..
//-Ordine nella variabili (spostate variabili dichiarate  ametà programma all'inizio, eccetto quelle blocco licenza e si danneggiano i file, così non occupano ram se il programma funziona correttamente, visto che sono molte)
//-Risoluzione bug minori
//-Migliporata sicurezza per evitare codici di risposta a quelli utente con numeri non concessi (visto che in base alla durata licenza cambiano, ora ho reso attivo in controllo in tutti e 3 i casi di dudrata degli anni della licenza)
...
//*****************************************************inizio dichiarazione varibili***************************************************************
   ...
   int lung_pcname=0,contatore=0;     //risolvono bug su percorso. inserito in beta 16.0.0
...
if (err != 0) printf_s("Errore x00B: Contattare ks89\n"); //non ho la minima idea di come come risolvere sto bug, perchè non ho la minima idea di come funzioni sto pezzo in c++
//*************************************************************************************************
//*************************************************************************************************
...
if (first_exec==1) {
					do{ //eseguo almeno una volta la generazione del codice, ma se alla fine mi creasse numeri non ammessi nella risposta rigenero il codice finchè le condizioni non mi vanno bene
                        ...
                        if ((cfPtr=fopen("ks.dat","rb+"))==NULL) printf("Errore x004\n");
							 ...
							 for(i=0;i<lung_pcname;i++) write.pcname[i]=pc_name[i]; //metto il nome pc codificato in numeri secondo ascii nel record per scrivere sul file
							 //***********************************************************************************************************
...
printf("             *****************************************************\n");
printf("             *..::Benvenuto in LIGHTWALL BETA 17.3.2 by ks89!::..*\n");
printf("             *****************************************************\n\n");
