...
//Richiede obbligatoriamente LCL beta 2.1.0 o 3.0.0 e License Manager beta 1.0.1 o superiore. Generaz risposta beta 5 è ancora supportato, ma non verrà più aggiornato, perchè già integrato in LCL beta 3.0.0


//nELLE PROSSIME VERSIONI
//-Capacità di leggere versioni di license.lic più vecchie e permetterne l'attivazione in modalità compatibilità
   //NB, questo sarà possbile grazie alla prima frase che ho scritto subito sotto a #include <stdio.h>. Con questo metodo mi assicurerò la retrocompatibilità delle licenze
   //Inoltre, Notare che solo le versioni superiori a 2.0.1 di license.dat sono utilizzabili da questa a tutte le future versioni
...
//NOVITA':
//-MOLTISSIME ED IMPORTANTTIME NOVITA' (prima versione perfettamente compatibile in tutto con LCL beta 3.0.0)
//-ricreato da capo la gestione percorso. La parte in c++ è identica, ma dopo non calcolo la lunghezza del percorso, copio sempre 100 caratteri o numeri tutte le volte che lavoro sul percorso, cioè la sua lunghezza massima ammessa
   //Tutte le variabili e il record write.path_codificato viene sempre resettato a 0, per far si che all'interno ci siano solo 0 e i valori del percorso e non numeri non ammessi come prima
   //Questo metodo è molto dispendioso in risorse, ma è mille volte più sicuro, perchè ora so sempre che cosa ho dentro alle variabili legate al percorso. prima avevo dei valori dei vettori e stringhe non visibili perchè calcolavo solo sulla lugnhezza e i valori superiori alla lunghezza erano impostato
   //dal compilatore a caso. Ora tutti quei valori casuali saranno sempre forzati a 0. per sicurezza tutte le volte che lavoro sui percorsi metto sempre a 0.
   //Con questa importante modifica HO RISOLTO dopo un sacco di tempo il bug gravissimo sul percorso che rendeva impossibile una registrazione da 0 con LCL da 1.0.0 a 3.0.0.
//-come per il percorso ricreato anche per nomepc forzando sempre tutto a 0. Ma non solo, dopo aver verificato che funzionassero ho semplificato tantissimo la gestione delle variabili pcname e path riducendole della metà
//-ottimizzazione variabili percorso e pcname
//-eliminazione procedura su pcname inserito in beta 16.0.0 per risolvere un bug! Ora avendo rivisto le procedure pcname e path quel bug non c'era più e quindi ho eliminato quel pezzo
//-Integrazione elevatissima con LCL beta 3.0.0 e test approfonditi su beta 17.3.3
//-verifica a fondo sulla compatibilità di license manager beta 1.0.1 OK! Funziona!
//-Riduzione possibili bug, miglioramenti vari al codice e rimozione di molte printf per scopi di test
...
printf("             *****************************************************\n");
printf("             *..::Benvenuto in LIGHTWALL BETA 17.3.3 by ks89!::..*\n");
printf("             *****************************************************\n\n");
...
if (scelta==1){
			  ...
			  /*IMPO!!*/if ((license.inutile_x==12344) && (license.inutile2_x==-546.451405) && (strcmp(license.reg_x,"jhfiusq")==0) && (isbn==1) && (j==1) && (verifica_seriale_file==30))
					    {
...
if (scelta==4)
{
	...
	if(license.tipolic==99) printf("\n La licenza in uso è PROFESSIONAL e non vi sono upgrade possibili, essendo già la migliore\n");
	else
	{//se è permesso fare l'upgrade entro qui, cioè solo se la licenza è home
		...
		{//se entro qui vuol dire che la licenza ha un codice corretto e posso stampare il codice seriale della licenza
			...
			lic_risposta_perverifica[5]=rand()%9;
...
else   //se licenza scaduta perchè superata la data di scadenza
{
	...
	lic_risposta_perverifica[0]=rand()%9;
	lic_risposta_perverifica[1]=(codice_seriale_licenza[10]+21)%9;
	lic_risposta_perverifica[2]=(codice_seriale_licenza[4]+7)%6+rinnova;
	lic_risposta_perverifica[3]=(codice_seriale_licenza[3]+3)%9;
	lic_risposta_perverifica[4]=(codice_seriale_licenza[1]+codice_seriale_licenza[0]+codice_seriale_licenza[5]+7)%9;
	lic_risposta_perverifica[5]=(codice_seriale_licenza[8]+codice_seriale_licenza[6])%9;
	lic_risposta_perverifica[6]=codice_seriale_licenza[2]%3+rinnova;
	lic_risposta_perverifica[7]=rand()%9;
	lic_risposta_perverifica[8]=(license.controllo_x+rinnova+2)%9;
	lic_risposta_perverifica[9]=(codice_seriale_licenza[7]+rand())%9;
	lic_risposta_perverifica[10]=(((7*codice_seriale_licenza[5])%(codice_seriale_licenza[6]+1))+codice_seriale_licenza[9])%9;
	lic_risposta_perverifica[11]=(codice_seriale_licenza[0]+lic_risposta_perverifica[2]+lic_risposta_perverifica[4])%9;
	...
	else
	{
		...
		if((rinnova==lic_risposta_perverifica[2]-(codice_seriale_licenza[4]+7)%6) && (lic_risposta_perverifica[8]==(license.controllo_x+rinnova+2)%9)) rinnova=lic_risposta_perverifica[6]-codice_seriale_licenza[2]%3;
