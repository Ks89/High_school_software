...
//VERSIONE BETA 3.0.1 di LCL compatibile solo da versione protezione lightwall beta 17.3.5 (TRA L'ALTRO ANCORA NON PIENAMENTE COMPATIBILE), non versioni precedenti


//PROSSIME MODIFICHE [ESSENZIALI] PER 3.1.0
//-AGGIUNGERE COMPATIBILTA' CON CODICE13 CHE IN QUESTA VERSIONE E' SOLO INSERITA PER RESETTARE CORRETTAMENTE I FILE, MA NON SI PUO' REGISTRARE E FAR FUNZIONARE TUTTO COMPLETAMENTE
  //-NB CON QUESTA VERSIONE NON SI PUO' REIGSTRARE DAL NULLA O ALTRE REGISTRAZIONI PERCHE' IL PROGRAMMA E' IN GRADO SOLO DI RESETTARE E NON CREARE CODICE13


//Miglioramenti rispetto versione 3.0.0
//-Modificate strutture per accogliere codice13
...
printf("         *************************************************************\n");
printf("         *..::Benvenuto in LIGHT COMMAND LINE BETA 3.0.1 by ks89!::..*\n");
printf("         *************************************************************\n\n");
...
if(strcmp(struttura,"restore")==0)
{
	if(strcmp(comando,"ks")==0)
	{
		...
		if ((cfPtr=fopen("ks.dat","rb+"))==NULL) printf("Errore L011\n");  //apro file in scrittura
				...
				for(i=0;i<13;i++) activate.codice13[i]=0;// inizializzo a 0 il campo codice13
				for (i=0;i<16;i++) write.md5[i]=md5_fun[i];
				for (i=0;i<20;i++) write.sha1[i]=sha1_fun[i];
				write.mixer1=1;
				write.mixer2=4;
