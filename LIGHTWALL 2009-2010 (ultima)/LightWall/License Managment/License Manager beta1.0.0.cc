...
//E' la prima versione, perfettamente funzionante

//PS: non attivare l'integrità MC sulla licenza! Per motivi di sicurezza è emglio che l'utente, una volta registrato il programma (che quindi può ancora ccedere a LicenseManagment) non sia a conoscenza di info così delicate
...
struct pathWData pathW={"",{0},0};    //record che uso per il percorso dell'eseguibile
	...
	if ((cfPtr=fopen("lic.dat","rb"))==NULL){
		printf("File lic non trovato. Errore z001\n\n");
	}
    else{
	...
	if ((cfPtr=fopen("ks.dat","rb"))==NULL){
		printf("File ks non trovato. Errore z001\n\n");
	}
    else{
...
printf("             ************************************************************\n");
printf("             *..::Benvenuto in LICENSE MANAGMENT BETA 1.0.0 by ks89!::..*\n");
printf("             ************************************************************\n\n");
printf("Scegli cosa fare\n");
printf("Inserisci '1' se vuoi vedere la licenza\n");
printf("Inserisci '2' se vuoi l'upgrade della licenza\n");
printf("\nAttenzione. Le opzioni 1 e 2 non sono disponibili in LIGHTWALL.exe dopo la registrazione\n");
scanf("%d",&scelta);
...
if (scelta==2)
{
	if(license.tipolic==99) printf("\n La licenza in uso e' PROFESSIONAL e non vi sono upgrade possibili, essendo già la migliore\n");
	else
	{//se è permesso fare l'upgrade entro qui, cioè solo se la licenza è home
		...
		printf("\n %d %d %d %d",write.codice[5],write.codice[6],8-(((activate.codice_a0*7)+(activate.codice_a2*6)+(activate.codice_a3*5)+(activate.codice_a4*4)+(activate.codice_a5*3)+(activate.codice_a6*2))%8),7-(((activate.codice_a0*6)+(activate.codice_a1*5)+(activate.codice_a2*4)+(activate.codice_a3*3)+(activate.codice_a4*2))%7));
