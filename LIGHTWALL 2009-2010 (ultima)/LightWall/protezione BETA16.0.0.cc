...
//BETA 16.0.0 SOURCECODE, sembra funzionare davvero!!!!!! Inoltre va bene sia su WINDOWS 7 x86 che x64!!! OTTIMO DAVVERO!
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
//Note: Tra le ultime versioni di LightWall questa è quella che ha richiesto più tempo, quasi un settimana per riuscire a far funzionare tutto. Infatti appena miglioravo una cosa non ne andava più un'altra
//Inoltre, ho dovuto far si che ripristino fabbrica e LCL fossero compatibili con i file creati da beta 16.0.0
//La versone 16.0.1 ha 2 piccolissime modifiche non sostanziali, di scarsa importanza.

//NOVITA':
//-Corretto bug mai notato nella creazione del percorso in int da salvare, che teneva conto anche del \0. Ovviamente dando dumero assurdo
//-Corretto bug mai notato nel salvataggio del nome pc su ks.dat: Che salva sempre 15 caratteri, ma se nome ha meno caratteri salva dei numeri assurdi alla fine fino ad arrivare a 15
//-Aggiunto md5 e sha1 funzionanti con correzione direttamente dentro questa versioni di tutti i bug legati ad essi (ce ne sono stati tantissimi, vedi i NB che servono x evitare altri errori)
//-
...
//nuovo bug RARISSIMO, succede una volta ogni centianio credo, da tanto che raro.
...
struct activate_licData{  //record per scrivere sul file, con acctname fissato ad 1, mentre i campi inutili  non servono a nulla, fa solo confusione per chi cercasse di cracckare
	   ...
	   int codice_a3;
	   int pcname_a[20];
	   //int path_codificato_a[100];  //questa cosa non è mai servita ad un grandissimo tubo
	   int codice_a4;
...
//*****************************************************inizio dichiarazione varibili***************************************************************
   ...
   int codice[7];           //codice è quello che uso per elaborare i dati. Questo diventa quello letto scritto con dei cicli for. Così sono sicuro di non toccare il record dei file quando non serve
   int risposta[12],scelta; //risposta ha il codice di risposta in caratteri usato per elaborare, che è diverso da risp_s, cioè la stringa
   ...
   char risp_s[12];         //questa è la stringa che si acquisice da tastiera con dentro il codice di risposta, poi viene convertina in intero e assegnata a risposta[12] per poi poter fare le elaborazioni
...
if (first_exec==1) {
      ...
      fclose(cfPtr);
                        ...
                        for(i=0;i<7;i++) codice[i]=write.codice[i];   //cioè metto in un array i dati che ho già scritto per elaborare dopo senza rischiare di toccare il file
...
if (write.controllo==7-(((write.codice[0]*6)+(write.codice[1]*5)+(write.codice[2]*4)+(write.codice[3]*3)+(write.codice[4]*2))%7)) isbn=1;   //è 1 se il codice di controllo è ok. 
else isbn=0;

if (activate.controllo_dat==7-(((activate.codice_dat[0]*6)+(activate.codice_dat[1]*5)+(activate.codice_dat[2]*4)+(activate.codice_dat[3]*3)+(activate.codice_dat[4]*2))%7)) isbn2=1;   //è 1 se il codice di controllo è ok. 
else isbn2=0;
...
if ((strcmp(writepc.nomepc_w,nome_pc)==0) && (strcmp(pathW.path_pathW,directory)==0) && (controllo_nomepc==1) && (controllo_percorso==1) && (write.inutile==12344) &&
	(write.inutile2==-546.451405) && (isbn2==1) && (activate.inutile_a==12344) && (activate.inutile2_a==-546.451405) && (h==100) && (indsha==20) && (indmd5==16))
{//QUI APRO L'IF GIGANTESCO CHE SI CHIUDE MOLTO PIU' AVANTI

printf("             *****************************************************\n");
printf("             *..::Benvenuto in LIGHTWALL BETA 16.0.0 by ks89!::..*\n");
printf("             *****************************************************\n\n");
...
if (scelta==1){
              ...
              if ((cfPtr=fopen("license.lic","rb"))==NULL) printf("File di licenza non trovato. Errore x015\n");
				...
				if ((cfPtr=fopen("lic.dat","rb"))==NULL) printf("File lic non trovato. Errore x016\n");
                    ...
                    if (license.controllo_x==7-(((license.codice_x[0]*6)+(license.codice_x[1]*5)+(license.codice_x[2]*4)+(license.codice_x[3]*3)+(license.codice_x[4]*2))%7)) isbn=1;  //è 1 se il codice di controllo è ok.
					else isbn=0;
...
if (strcmp(write.reg,c1)==0){//apre un if che finisce sotto, dopo un bel pezzo di programma
  ...
  scanf("%s",risp_s); 
  for(i=0;i<12;i++) risposta[i]=(int)risp_s[i]-48;                            // con questo trasformo i caratteri(che poi sono numeri) inseriti nella stringa in interi, sottraendo 48, cioè il valore dello 0 in ASCII
    
  //ora controllo che la cifra di controllo sia corretta
  if (write.controllo==7-(((codice[0]*6)+(codice[1]*5)+(codice[2]*4)+(codice[3]*3)+(codice[4]*2))%7)) isbn=1;	//è 1 se il codice di controllo è ok.
  else isbn=0;
  ...
  { //se tutti veri significa che la risposta è corretta
		     ...
		     /*IMPO!!*/  printf("\n\nCodice corretto\n");    // qui teoricamente ,ci dovrebbe essere tutto il programma al quale vorrei applicare il sistema di protezione
                         if ((cfPtr=fopen("ks.dat","rb+"))==NULL) printf("File ks.dat non trovato. Errore x031\n");
                                ...
                                write.controllo=7-(((codice[0]*6)+(codice[1]*5)+(codice[2]*4)+(codice[3]*3)+(codice[4]*2))%7);	//cioiè riscivo la cifra perchè la registrazione è corretta
                                write.inutile=12344;
                                write.mixer1=1;
                                write.mixer2=4;
                                write.mixer3=8;
                         ...
                         if ((cfPtr=fopen("license.lic","w"))==NULL) printf("Errore x032\n");
                              ...
                              if ((cfPtr=fopen("license.lic","rb+"))==NULL) printf("Errore x033\n");
                                     ...
                                     license.controllo_x=7-(((codice[0]*6)+(codice[1]*5)+(codice[2]*4)+(codice[3]*3)+(codice[4]*2))%7); //cioiè riscivo la cifra perchè la registrazione è corretta
...
/*IMPO!!*/ if ((write.inutile==12344) && (write.inutile2==-546.451405) && (strcmp(write.reg,c2)==0)) {
      ...
      if ((cfPtr=fopen("ks.dat","rb"))==NULL) printf("Errore x035\n");    //leggo file
           ...
           fread(&write,sizeof(struct writeData),1,cfPtr); 
           //ora controllo che la cifra di controllo sia corretta, se corretta, bene eseguo programm
           if (write.controllo==7-(((write.codice[0]*6)+(write.codice[1]*5)+(write.codice[2]*4)+(write.codice[3]*3)+(write.codice[4]*2))%7)) isbn=1;                           //è 1 se il codice di controllo è ok. 
     ...
     if((license.inutile_x!=12344) || (license.inutile2_x!=-546.451405) || ((license.num_domanda_x>6) || (license.num_domanda_x<0)))
	 {
	    ...
	    if((cfPtr=fopen("license.lic","rb"))==NULL) printf("\nImpossibile eseguire il ripristino senza license.lic\n\n\n");
	      ...
	      else
	      {
	        ...
	        isbn=7-(((write.codice[0]*6)+(write.codice[1]*5)+(write.codice[2]*4)+(write.codice[3]*3)+(write.codice[4]*2))%7);   //è 1 se il codice di controllo è ok. 
            ...
            { //vuol dire che lic,dat è danneggiato ma non ks.dat, che invece è a posto
		       ...
		       if ((cfPtr=fopen("lic.dat","w"))==NULL) printf("Errore x042");
		            ...
		            if ((cfPtr=fopen("lic.dat","rb+"))==NULL) printf("Errore x043");  //scrivo il codice sul file riscrivendo anche tutti gli altri dati per sicurezza
                          ...
                          for(i=0;i<7;i++) activate.codice_dat[i]=0; //però anche il codice utente qui lo metto alla cazzo,così so che questo è danneggiato di sicuro
