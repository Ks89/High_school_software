...
//Richiede obbligatoriamente LCL beta 2.0.x e anche GENERAZIONE RISPOSTA BETA 5 (le versioni prima non funzionano) e License Managment beta 1.0.0 o superiore
...
//NOVITA':
//-se registro prog da licenza ora è in grado di capire il tipo di licenza (HOME O PROF) leggendo dal file license.lic e impostando tutto come si deve
//-corretto difetto nella stampa stato registrazione se faccio scelta di vedere la licenza. Di conseguenza corretto anche in License Managment beta 1.0.0
//-Modificata struttura licenza con campo versioni sempre al primo membro della struttura per assicurare retrocompatibilità e modalità upgrade con correzione dei BUG
...
void hash_license_funzione(int codice_fun[],int controllo_fun,char nome_fun[],char cogn_fun[],char soc_fun[],char mail_fun[],char reg_fun[],int num_domanda_fun,int domanda_fun[],char version_fun[])
{
   ...
   //inizio procedura per salvare md5 e sha1 nel file license
   //inizio  nel gestire prima le strighe, che soon più facile da concatenare
   ...
   //ora devo concatenare la risposta alla domanda
   //so che mi trovo a dover mettere i dati a ind+lungh_hash+2, per semplicità inizializzo una variabile a questo valore
...
if (first_exec==1) {
					do{ //eseguo almeno una volta la generazione del codice, ma se alla fine mi creasse numeri non ammessi nella risposta rigenero il codice finchè le condizioni non mi vanno bene
						...
						if ((cfPtr=fopen("rek.dll","w"))==NULL) printf("Errore x009a\n");
					    else{
					         if ((cfPtr=fopen("rek.dll","rb+"))==NULL) printf("Errore x009b\n");
					         else {                              
						           fseek(cfPtr,0,SEEK_SET);
						           fwrite(&writepc,sizeof(struct writepcData),1,cfPtr); 
						           fclose(cfPtr); 
						...
						//risalvo file ks ecc.. e anche md5 e sha-1 generati dalla funzione sopra
						if ((cfPtr=fopen("ks.dat","w"))==NULL) printf("Errore x009c\n");
					    else{
					         if ((cfPtr=fopen("ks.dat","rb+"))==NULL) printf("Errore x009d\n");
					         else {                              
									fseek(cfPtr,0,SEEK_SET);
									fwrite(&write,sizeof(struct writeData),1,cfPtr); 
									fclose(cfPtr);  
						...
						//risalvo file lic ecc.. e anche md5 e sha-1 generati dalla funzione sopra
						if ((cfPtr=fopen("lic.dat","w"))==NULL) printf("Errore x009c\n");
					    else{
					         if ((cfPtr=fopen("lic.dat","rb+"))==NULL) printf("Errore x009d\n");
					         else {                              
									fseek(cfPtr,0,SEEK_SET);
									fwrite(&activate,sizeof(struct activate_licData),1,cfPtr); 
									fclose(cfPtr);  
...
printf("             *****************************************************\n");
printf("             *..::Benvenuto in LIGHTWALL BETA 17.3.1 by ks89!::..*\n");
printf("             *****************************************************\n\n");
...
if (controllo_registrazione_variabile==22){ // fa si che se già registrato non richieda la modalità di registrazione. A 32 vuol dire registrato
			...
			printf("\nAttenzione. Le opzioni 3 e 4 sono disponibili in License beta 1.0.0.exe\nanche dopo la registrazione\n");
			scanf("%d",&scelta);
			}
else scelta=2;
...
if (scelta==1){
              ...
              if ((cfPtr=fopen("license.lic","rb"))==NULL) printf("File di licenza non trovato. Errore x015\n");
				...
				if ((cfPtr=fopen("lic.dat","rb"))==NULL) printf("File lic non trovato. Errore x016\n");
					...
					int contr_sha_license=0,contr_md5_license=0;
			  ...
			  /*IMPO!!*/if ((license.inutile_x==12344) && (license.inutile2_x==-546.451405) && (strcmp(license.reg_x,c2)==0) && (isbn==1) && (j==1) && (verifica_seriale_file==30))
						...
						//risalvo file ks  ecc.. e anche md5 e sha-1 generati dalla funzione sopra
						if ((cfPtr=fopen("ks.dat","w"))==NULL) printf("Errore x007\n");
					    else{
					         if ((cfPtr=fopen("ks.dat","rb+"))==NULL) printf("Errore x008\n");
					         else {                              
						           fseek(cfPtr,0,SEEK_SET);
						           fwrite(&write,sizeof(struct writeData),1,cfPtr); 
						           fclose(cfPtr); 
						...
						//risalvo file irr ecc.. e anche md5 e sha-1 generati dalla funzione sopra
						...
						//risalvo file lic ecc.. e anche md5 e sha-1 generati dalla funzione sopra
						if ((cfPtr=fopen("lic.dat","w"))==NULL) printf("Errore x007\n");
					    else{
					         if ((cfPtr=fopen("lic.dat","rb+"))==NULL) printf("Errore x008\n");
					         else {                              
						           fseek(cfPtr,0,SEEK_SET);
						           fwrite(&activate,sizeof(struct activate_licData),1,cfPtr); 
						           fclose(cfPtr); 
		...
		{//se entro qui vuol dire che la licenza ha un codice corretto e posso stampare il codice seriale della licenza
			...
			else
			{
				...
				{	//questo fa si che se voglio fare upgrade, la licenza scadrà l'anno in cui ho registrato la licenza la prima volta +1(2 o 3)
                 ...
                 if ((cfPtr=fopen("license.lic","rb+"))==NULL) printf("Errore z003\n");
                 else {
                          fseek(cfPtr,0,SEEK_SET);
                          fwrite(&license,sizeof(struct licenseData),1,cfPtr); 
                          fclose(cfPtr); 
                      }
				 if ((cfPtr=fopen("lic.dat","rb+"))==NULL) printf("Errore z003\n");
                 else {
                          fseek(cfPtr,0,SEEK_SET);
						  fwrite(&activate,sizeof(struct activate_licData),1,cfPtr); 
                          fclose(cfPtr); 
