...
//ATTENZIONE MOLTO IMPO: QUESTA VERSIONE E' COMPATIBILE FINO A BETA 15.7.4! FUNZIONA SOLO GRAZIE ALLA FORTUNA. PERCHE' IN QUELLE VERSIONI LE STRUTTURE SONO CAMBIATE
//PERO' IL CAMBIAMENTO DELLA 16 E' TALE DA FAR SI CHE QUESTA VERSIONE NON SIA PIU' UTILIZZABILE, COSI' RISCRIVO TUTTO AGGIORNANDO LE STRUTTURE E FACENDO CANCELLARE
//VERAMENTE I DATI (CHE IN QUESTA VERSIONE NON CANCELLAVA DAVVERO) E SOPRATTUTTO REIMPOSTARE I MD5 E SHA-1 (PER QUESTO VEDI BETA 8)
...
struct writeData{   //record per scrivere sul file, con acctname fissato ad 1, mentre i campi inutili  non servono a nulla, fa solo confusione per chi cercasse di cracckare
	   ...
	   int mixer4;
	   char reg[5]; //sono variabili che fanno incasinare (mescolare) i dati nel file, così è difficile capire quali sono dati utili e soprattutto uso questi dati per fare controlli su integrità file(sblocco licenza)
       int mixer5;
