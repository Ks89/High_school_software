...
//versione beta7, working progress!!!!
//BUG NOTI versione precedente:
//-(grave) la lettura del file si basa sulla lunghezza della lista. ma se la lista non è stata ancora creata non posso leggere.quindi posso leggere solo dopo che ho scritto sul file
//-(ridottissimo) [non capisco quando succede] nonostante le correzzioni continue il meccanismo di "delet" da problemi e cancella cosa sbagliate. Correggerlo
//-(non bug) se non ho creato una lista(appena vviato programma) premo 7 crasha. non è grave, tanto si presume che uno prima di salvare inserisca dei dati. Soluzione possibile = se *sPtr->successivo==NULL creo un elemento con dentro il nulla. Appena aggiungo un elemento cancello questo e metto quelli nuovi. in modo tale che ci sia sempre + di un elemento
...
int main()
{
    ...
    char stringa[35],stringa2[35]; //uno il nome e l'altro il cognome
  	int i,cont,cicli,indice,numeri=1;
	int posiziones,len; 
	char datos[15],cognomes[35],nomes[35];
      ...
      switch(choice){
            ...
            case 3:
			 ...
			 case 6:
				  ...
				  break;
			 case 7: //scrive i dati sul file rubrica.dat
				  if((cfPtr=fopen("rubrica.dat","w"))==NULL) printf("File non apribile\n"); //apre file in scrittura
				  else {
						i=1;
						cont=0;
						currentPtr=startPtr;
					    while(currentPtr!=NULL && cont!=i) //questo while serve a contare il numero di elementi nella lista
						{
						...
						cicli=i;
						currentPtr=startPtr;
						while(cont<cicli)  //ora scrivo i dati sul file
						{	
							posiziones=currentPtr->posizione;
							for(i=0;i<15;i++) datos[i]=currentPtr->dato[i];

							for(i=0;i<35;i++){ //dato che ho problemi nel leggere stringhe con spazi elimino spazi e metto @, poi alla lettura dove vedo @ ci stampo uno spazio
							    if(currentPtr->cognome[i]==32)  cognomes[i]='@';
								else cognomes[i]=currentPtr->cognome[i];
							}

							for(i=0;i<35;i++){
								if(currentPtr->nome[i]==32)  nomes[i]='@';
								else nomes[i]=currentPtr->nome[i];
							}
							fprintf(cfPtr,"%d %s %s %s\n",posiziones,cognomes,nomes,datos);
						    cont++;
							currentPtr=currentPtr->successivo;
						 }//chiude while
						 fclose(cfPtr);
				  ...
				  break;
			 case 8:
				  if ((cfPtr=fopen("rubrica.dat","r"))==NULL) printf("File non apribile\n");     
				  else {
					   i=1;
					   cont=0;
					   currentPtr=startPtr;
					   while(currentPtr!=NULL && cont!=i) //come prima calcola il numero di elementi
					   {
							currentPtr=currentPtr->successivo;
							i++;
							if (currentPtr->successivo==NULL) cont=currentPtr->posizione;
					  ...
					  while(cicli<cont+1)
					  {
					        fscanf(cfPtr,"%d %s %s %s",&posiziones,cognomes,nomes,datos);
						    printf(">> %d : ",posiziones);
							
							cont_2=0; //ora calcolo la lunghezza del cognome per poi fare un for
							while(cognomes[cont_2]!='\0') cont_2++;

							for(i=0;i<cont_2;i++) {
													if(cognomes[i]!='@') printf("%c",cognomes[i]);
													else printf(" ");
							                      }
							printf(" ");
							cont_2=0; //ora calcolo la lunghezza del nome per poi fare un for e stamparlo a schermo
							while(nomes[cont_2]!='\0') cont_2++;
