...
//versione beta6, working progress!!!!
//BUG NOTI DI QUESTA VERSIONE:
//-(grave) se metto cognomi o nomi con spazi il salvataggio su file è ok, ma la lettura va in crisi
...
//-(medio) nonostante le correzzioni continue il meccanismo di "delet" da problemi e cancella cosa sbagliate. Correggerlo
            ...
            case 3:
			 ...
			 case 7: //scrive i dati sul file rubrica.dat
				  if((cfPtr=fopen("rubrica.dat","w"))==NULL) printf("File non apribile\n"); //apre file in scrittura
						...
						{	
							...
							for(i=0;i<35;i++) cognomes[i]=currentPtr->cognome[i]; 
							for(i=0;i<35;i++) nomes[i]=currentPtr->nome[i];
			 ...
			 case 8:
				  if ((cfPtr=fopen("rubrica.dat","r"))==NULL) printf("File non apribile\n");     
					  ...
					  while(cicli<cont+1)
					  {
					        fscanf(cfPtr,"%d %s %s %s",&posiziones,datos,cognomes,nomes);
						    printf(">> %d : %s %s -> %s\n\n",posiziones,datos,cognomes,nomes);
						    posiziones=0;
					        cicli++;
					  }//chiude while
					  fclose(cfPtr);
